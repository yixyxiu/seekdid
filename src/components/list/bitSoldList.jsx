...
const BitSoldList = () => {
    ...
    async function asyncGetSoldAccountList() {
        let url = `https://api.das.la/api/v2/das_accounts/get_bit_sales_data?limit=10&type=3`
    ...
    return (
        <div className='flex flex-col rounded-lg p-5 hover-up-2 bg-main-card min-w-[370px] md:max-w-[380px] min-h-[310px] bg-box-shadow'>
            ...
            {data.loading ? (skeletonRows.map((item, index) => <SalesListSkeleton key={`sold-list-placeholder-${index}`} />))
            :(<div className='mt-2 flex flex-col justify-left'>
            {
                    data.accountList?.map((item,i) => {
                        return <a key={`${item.account}-${i}`} href={getAccountMarketLink(item.from, item.account)} rel="noopener noreferrer" target="_blank"><div key={`${i}-${item.account}`} className={`flex flex-row cursor-pointer common-bitlist border-b-[1px] border-line seperator ${isNewItem(item.account) ? 'new_item_animation':' '}`}>
